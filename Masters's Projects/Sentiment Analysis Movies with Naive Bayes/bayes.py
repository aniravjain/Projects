...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    """
    V = defaultintdict()
    flat_neg = [ item for elem in neg for item in elem ]
    flat_pos = [ item for elem in pos for item in elem ]
    combined = flat_neg + flat_pos
    uniq_val = np.unique(sorted(combined))
    V['__unknown__'] = 0
    for i in range(0,len(uniq_val)):
        V[uniq_val[i]] = i+1
    #print(type(V))
    return V
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    """
    # YOUR CODE HERE
    t_li = [vectorize(V,i) for i in docs]
    D = np.stack(t_li)
    return D
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        neg_X = X[neg_index]
        pos_X = X[pos_index]
        self.prob_neg = len(neg_X)/len(X)
        self.prob_pos = len(pos_X)/len(X)
        self.words_in_neg = np.sum(neg_X)
        self.words_in_pos = np.sum(pos_X)
        self.prob_neg_words = (np.sum(neg_X, axis=0)+1)/(self.words_in_neg + len(X[0]))
        self.prob_pos_words = (np.sum(pos_X, axis=0)+1)/(self.words_in_pos + len(X[0]))
        self.log_prob_neg_words = np.log(self.prob_neg_words)
        self.log_prob_pos_words = np.log(self.prob_pos_words)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        # YOUR CODE HERE
        self.prob_neg = np.log(self.prob_neg) + np.dot(X, self.log_prob_neg_words)
        self.prob_pos = np.log(self.prob_pos) + np.dot(X, self.log_prob_pos_words)
        y_pred = np.empty(len(self.prob_neg))
        for i in range(len(y_pred)):
            if (self.prob_neg[i]>self.prob_pos[i]):
                y_pred[i] = 0
            else:
                y_pred[i] = 1
        return y_pred
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    """
    kf = KFold(n_splits=k,shuffle=True,random_state=999) # use same split every time for student eval
    accuracies = []
    for i,j in kf.split(X=X,y=y):
        model.fit(X[i],y[i])
        y_pred=model.predict(X[j])
        accuracies.append(np.sum(y[j] == y_pred)/len(y[j]))
