...
def minimize(X, y, loss_gradient,
    ...
    for i in range(max_iter):
        ...
        h+=np.square(loss)
        B=prev_B-eta*loss/(np.sqrt(eps+h))
        prev_B=B
        if np.linalg.norm(loss_gradient(X,y,B,lmbda))<=precision:
            break
    return B
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        ...
        init_B=minimize(X,y,loss_gradient_ridge,self.eta,self.lmbda,self.max_iter,False)
        init_B0=np.mean(y)
        self.B=np.vstack([init_B0,init_B])